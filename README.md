...
v0.1.9 적용 후에도 가방 오른쪽 정렬 카테고리 `일반/퀘스트/골드`가 네모로 보이던 문제를 우회 수정했습니다.

- 기존 번역 22,488행은 유지했습니다.
- 해당 버튼 prefab은 다른 정상 아이템 텍스트와 같은 font asset을 쓰지만, 이 표면에서만 한글 동적 glyph 갱신이 실패하는 것으로 확인했습니다.
- 박스 표시를 막기 위해 해당 3개 라벨만 `Normal`, `Quest`, `Gold`로 표시합니다.
- 새 Excel 패치 결과 SHA256: `196ad86bbbb018fb10357c00364f83fcba735a0541345195be1956a2c17eb593`
- v0.1.9처럼 이전 패치 해시가 이미 적용된 상태에서도 새 target이 다르면 다시 적용하도록 설치 로직을 보강했습니다.
