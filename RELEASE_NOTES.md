# Tales of Seikyu 비공식 한글패치 설치/업데이트/제보 안내

안녕하세요.

Tales of Seikyu 비공식 한글패치 배포 안내입니다.

## 1) 최신 버전

- 패치 버전: `0.1.0-playtest.20260621`
- 지원 플랫폼: Steam Windows
- 대상 게임 버전/buildid: 2026년 6월 17일 이후 현재 로컬 검증 빌드의 확인된 Addressables 번들 해시
- 최근 갱신일: 2026-06-21

## 2) 다운로드

GitHub Releases에서 `TalesOfSeikyuKoreanPatch-v0.1.0.zip`을 다운로드해 주세요.

다른 곳으로 공유할 때는 파일 재업로드 대신 이 저장소/릴리스 링크를 공유해 주세요.

## 3) 설치 방법

1. ZIP 파일을 다운로드합니다.
2. 원하는 위치에 압축을 풉니다.
3. `TalesOfSeikyuKoreanPatch.exe`를 실행합니다.
4. 게임 폴더가 자동으로 잡히지 않으면 `찾아보기`로 설치 폴더를 지정합니다.
5. `한국어 패치 설치/업데이트`를 실행합니다.
6. 완료 후 게임을 다시 실행해 한글 적용 여부를 확인합니다.

중요:

- 게임을 종료한 상태에서 설치/복구를 진행해 주세요.
- 패처가 지원하지 않는 해시라고 표시하면, 게임 업데이트 또는 다른 패치 상태일 수 있습니다.

## 4) 제거(원본 복구) 방법

1. `TalesOfSeikyuKoreanPatch.exe`를 실행합니다.
2. `원본 복구`를 실행합니다.
3. 설치 시 생성된 백업으로 복구됩니다.

복원이 정상적으로 되지 않는다면 Steam 파일 무결성 검사를 사용해 주세요.

## 5) 주요 번역 범위

- 시스템/UI
- 메뉴/설정
- 튜토리얼/도움말
- 퀘스트/목표
- 대화/이벤트
- 아이템/설명
- 일지/기록물/로어
- 기타 문자열

## 6) 자동으로 진행되는 작업

- 게임 설치 경로 확인
- 지원 번들 해시 확인
- 원본 파일 백업 생성
- 한국어 텍스트 적용
- 가방 UI 카테고리용 한글 폰트 fallback 적용
- 패치 적용 후 검증

패치 파일에는 완성된 게임 원본 번들 전체가 들어 있지 않습니다. 사용자 PC에 설치된 원본 게임 파일에 한글 텍스트를 적용하는 방식입니다.

## 7) 알려진 이슈

- 게임 업데이트로 파일 구조나 해시가 바뀌면 패치가 동작하지 않을 수 있습니다.
- 일부 문장은 실제 플레이 문맥을 보며 계속 다듬는 중입니다.
- 아직 충분히 플레이테스트하지 못한 화면에서 줄넘침이나 문맥 어색함이 있을 수 있습니다.

## 8) 플레이테스트 제보

아래 유형을 발견하면 스크린샷과 함께 제보해 주세요.

- 문맥이 어색한 번역
- 말투/호칭 이탈
- UI 줄넘침 또는 잘림
- 영어/일본어 잔존
- 내부 키/태그 노출
- 오타/오번역
- 고유명사 표기 흔들림
- 설치 실패 또는 패처 오류

## 9) 안내 및 면책

- 본 패치는 비공식 팬 번역이며 개발사/퍼블리셔와 무관합니다.
- 게임 원본 파일 및 권리는 각 권리자에게 있습니다.
- 정식 한국어 출시 또는 권리자 요청 시 배포를 중단/수정할 수 있습니다.
- 패치 파일 외 게임 원본 자산 전체는 배포하지 않습니다.
- 상업적 판매/유료 배포를 금지합니다.

감사합니다.

좋은 플레이 경험을 위해 계속 개선하겠습니다.
